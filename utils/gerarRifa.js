...
export default async function gerarRifa(rifa, taskId, io) {
  const template = fs.readFileSync(
    `${templatesPath}/rifa_${rifa.template}l.html`,
    "utf8",
  );
  const templateListaVendedores = fs.readFileSync(
    `${templatesPath}/lista_vendedores.html`,
    "utf8",
  );
  ...
  if (rifa.vendedor) {
    ...
    const folhasVendedorExtra = Math.ceil(
      Number(rifa.folhasExtra) / (35 * Number(rifa.folhasPorVendedor)),
    );
    const folhasDeVendedores = turmas + folhasVendedorExtra;

    totalFolhas =
      folhasDeVendedores +
      Number(rifa.folhasExtra) +
      Number(rifa.folhasPorVendedor) * Object.values(lista).flat().length;
    ...
    for (let i = 1; i <= folhasVendedorExtra; i++) {
      if (i == 1)
        paginasVendedores.push(
          totalFolhas - Number(rifa.folhasExtra) - folhasVendedorExtra,
        );
    ...
    vendedores.push(
      ...Array(Number(rifa.folhasExtra)).fill("__________________"),
    );
  } else {
  ...
  let ultimoNumero = 1;
  const soma = rifa.vendedor
    ? Number(rifa.template) * Number(rifa.folhasPorVendedor)
    : undefined;
  ...
  for (let i = 0; i < totalFolhas; i++) {
    ...
    if (paginasVendedores.includes(i)) {
      ...
      if (index >= Object.keys(lista).length) {
        html = html.replace(/{{TURMA}}/g, "<em>Folhas Extra</em>");
        ...
        html = html.replace(/{{TURMA}}/g, Object.keys(lista)[index]);
        ...
        for (let n = 1; n <= 35; n++) {
          ...
          html = html
            .replace(
              nomeRegex,
              Object.values(lista)[index]
                ? (Object.values(lista)[index][n - 1] ?? "&nbsp;")
                : "&nbsp;",
            )
            .replace(
              numeroRegex,
              Object.values(lista)[index]
                ? Object.values(lista)[index][n - 1]
                  ? `${ultimoNumero} - ${ultimoNumero + (soma - 1)}`
                  : "&nbsp;"
                : "&nbsp;",
            );
      ...
      html = template
        .replace(
          /{{VENDEDOR}}/g,
          rifa.vendedor ? vendedores[i] : "__________________",
        )
        .replace(/{{RIFA}}/g, rifa.nome.toUpperCase())
        ...
        .replace(
          /{{PREÇO}}/g,
          parseFloat(rifa.preco.replace(",", ".")).toLocaleString("pt-BR", {
