...
export default async function validarRifa(req) {
  ...
  if (
    !rifa.premiacao ||
    typeof rifa.premiacao !== "string" ||
    rifa.premiacao.length < 4 ||
    rifa.premiacao.length > 20
  ) {
  ...
  // Preço
  const preco =
    typeof rifa.preco === "string"
      ? parseFloat(rifa.preco.replace(",", "."))
      : rifa.preco;
  ...
  if (
    !rifa.template ||
    !templatesValidos.includes(String(rifa.template).trim())
  ) {
